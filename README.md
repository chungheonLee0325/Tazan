...
* **비트마스크 기반 상태 관리**: `EConditionBitsType` enum을 활용한 효율적인 32가지 캐릭터 상태 관리
* **타이밍 기반 Perfect 시스템**: 
...
### 📊 데이터 주도 설계
* **UDataTable 기반**: 코드 변경 없이 밸런스 조정 가능한 확장 가능한 구조
...
### 몬스터 AI
* **일반 몬스터**: 검 해골, 검 든 적, 할버드 맨 - 기본 FSM 패턴
...
### 🎯 정밀한 타이밍 시스템
**문제:** Perfect Guard/Dodge의 정확한 타이밍 윈도우 구현

**해결:**
- 비트마스크 기반 `EConditionBitsType`으로 다중 상태 효율적 관리
- `FTimerHandle`을 활용한 정밀한 시간 기반 상태 해제
...
| 역할 | 담당자 | 주요 기여 |
|------|--------|-----------|
| **메인 프로그래머** | 김희연 | Base Class 구현, 플레이어 시스템, 게임 시스템, Sword Skeleton, HalberdMan 구현 |
| **보스 AI 프로그래머** | 이충헌 | Yetuga(보스 몬스터) AI 및 스킬 시스템 구현 |
