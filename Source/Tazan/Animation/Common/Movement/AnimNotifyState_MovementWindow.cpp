...
static void AutoResolveTargetsIfNeeded(FAreaMoveSpec& S, AAreaObject* Area, bool bUseLockOnToward,
                                       bool bUseLockOnFacing)
{
	if (!Area) return;
                                                  ...
                                                  float TotalDuration, const FAnimNotifyEventReference& EventReference)
{
	...
	{
		// 플레이어 버퍼를 소비해서 Dir 확보
		const ETazanDir Dir = UMoveUtilComponent::ReadPlayerIntent(Area);
		...
		if (bUseCustomSpec)
		{
			...
			// 플레이어인 경우 스펙 완성 직전 덮어쓰기
			UMoveUtilComponent::ApplyDirectionalOverride(S, Area, Dir);
